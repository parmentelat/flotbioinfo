...
###OFF genes = coding_regions_one_phase(subtilis, 0)
###OFF print("On a trouvé {} gènes sur la phase 0".format(len(genes)))
...
###OFF array_of_lengths = [ y-x for x,y in genes ]

# la longueur totale de tous les gènes trouvés
###OFF total_length = sum ( array_of_lengths )
# la longueur moyenne des gènes
###OFF average_length = total_length / len(genes)
###OFF print('longueur moyenne des gènes', average_length)


# In[10]:

# les tailles minimale et maximale
###OFF length_min = min ( array_of_lengths )
###OFF length_max = max ( array_of_lengths )
###OFF print("min = {}, max = {}".format(length_min, length_max))


# In[11]:

# pourcentage de la région codante par rapport à la longueur totale
###OFF print("Pourcentage de région codante", total_length/len(subtilis))
...
###OFF get_ipython().magic(u'matplotlib inline')

# la taille utile pour les courbes
###OFF import pylab
###OFF pylab.rcParams['figure.figsize'] = 8., 8.


# In[13]:

# un histogramme de la répartition des longueurs 
###OFF plt.hist(array_of_lengths, bins=75)
###OFF plt.axis([300, 7700, 0, 400])
###OFF plt.show()
